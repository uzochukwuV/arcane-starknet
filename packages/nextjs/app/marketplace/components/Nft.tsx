  ...
  return <div className="pb nf hk/2 _k/3 2xl:ud-w-1/4">
    <div className="ja nd qd bg-indigo-50/40   wd hf">
      <div className="f ka gd kd">
        <Image src={nft?.url!} height={200} width={200} alt="auctions" className="pb" />
      ...
      <div>
          ...
          <div className="pb">
            <div className="za yc">
              {/* <div className="ta hb pb fc ld">
                                  <Image src="src/images/auctions/creator-01.png" alt="creator" className="ib pb af bf" />
                                  </div> */}
              <div className="pb">
        ...
        }} className="za yc _c cursor-pointer bg-slate-500 se kf eh nh ">
          BUY NFT
        </div>
      </div>
    </div>
...
function BuyNFT({ token_id, setBuying, token_cost }: any) {
  ...
  useEffect(() => {
    let payprice = pay == 1 ? Number(token_cost) * 100000000 / Number(ethprice?.toString()) : pay == 2 ? Number(token_cost) * 100000000 / Number(strkprice?.toString()) : Number(token_cost) * 100;
    setPrice(payprice)
  }, [pay, ethprice, strkprice, token_cost])


  const { sendAsync: ApproveEth } = useScaffoldWriteContract({
    contractName: "Eth",
    functionName: "approve",
    args: [arcane_address, price * WEI],
  });
  ...
  const buy_nft = async () => {
    try {
      const trx = pay == 1 ? await ApproveEth() : pay == 2 ? await ApproveStark() : await ApproveArc();
      const trxBuy = await BuyNft()
      setBuying(false)
    } catch (error) {
      console.log(error);
  ...
  return (
    <div className=" z-40 fixed top-0 left-0 right-0 bottom-0 bg-black/5 flex justify-center items-center">
      ...
      <div className="4 max-w-lg w-full bg-white px-6 pb-6 rounded-md flex flex-col gap-6 border border-slate-300 shadow-lg py-6">
        <button onClick={() => setBuying(false)} className=" text-black" >close</button>
        <h1>Pay with </h1>

        <button onClick={() => { setPay(1) }} className="px-12 py-3 cursor-pointer rounded-md border border-gray-300 text-slate-800 font-medium">
          {Number(token_cost) * 100000000 / Number(ethprice?.toString())}  Eth (default) {pay == 1 && "ticked"}
        </button>
        <button onClick={() => { setPay(2) }} className="px-12 py-3 cursor-pointer rounded-md border border-gray-300 text-slate-800 font-medium">
          {Number(token_cost) * 100000000 / Number(strkprice?.toString())}  Strk {pay == 2 && "ticked"}
        </button>
        <button onClick={() => { setPay(3) }} className="px-12 py-3 cursor-pointer rounded-md border border-gray-300 text-slate-800 font-medium">
          {Number(token_cost) * 100}  Arc {pay == 3 && "ticked"}
        </button>

        <button onClick={buy_nft} className="za yc _c cursor-pointer  kf eh nh bg-slate-200 ">
          Pay
        </button>
      </div>
    </div>
